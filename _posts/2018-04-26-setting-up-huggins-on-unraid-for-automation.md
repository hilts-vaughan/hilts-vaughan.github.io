...
**The process of ordering something that priced in error**

To get in on this, you need to...

1. Notice and find out about the error. Usually communities like RedFlagDeals or SlickDeals have communities who tell each other about these things. There are also services that notify people (more on this later)
2. You need to order and act quick.

You need a quick reaction time as these are often fixed within an hour, sometimes even less than that. There's other factors as well, such as in Canada, if you can do in an in-store  reservation at that price and then go in to get it, you have better odds than having it shipped, since orders are sometimes canceled. 

We're going to use **Huginns** to help us with step 1 -- since short of reading every thread frequently, finding them is not easy. To see how, 

<aside>

[If you don't know what Huginn is you can read more about it here.](https://github.com/huginn/huginn) It's essentially a system that allows you to pull data from various sources, aggregate it into events, query it, and then act on it. That sort of vague, right? So, what if I told you it could help you sales like this:
...
No single strategy is going to get you 100% of there and some services provide automated ways of doing this already for you. I was a frequent user of #2 for a while but it takes up a lot of time, since often they are the best sources of some of the more obscure deals. Wouldn't it be nice if we could just get a notification when someone posts something?
...
They both have some problems, though:

1. They're a bit slow -- it can sometimes take up to 15 minutes to get the e-mail and that's a bit too slow in some cases (despite saying "instant" in some cases)
...
3. They don't support complicated queries or Regular Expressions, which as a developer, I know is pretty important in some cases. ;)

We know the inflexibility problem in this case is because the "data source" and the notification system (and querying of the data) are coupled to each other.
...
If you don't have your own infrastructure, it's not a bad way to go. But let's look at alternatives using _Huginn_ to do this. I'm not going to cover the installation, so check out their [install guide](https://github.com/huginn/huginn/blob/master/doc/manual/installation.md) or my install guide using [unRAID](http://vaughanhilts.me/blog/2018/04/26/setting-up-huggins-on-unraid-for-automation.html) if you want to follow along. Otherwise, time to get started.

# Setting up a plan

This is our general plan of attack:

1. Monitor the RSS feed of a particular forum and pull in the **new posts**
2. Aggregate them all into a pool
3. Run some regular expressions against each entry to filter down to the ones we're interested in
4. Dispatch to a notification daemon and have it notify me in real time

We're going to do with this a few agent rules. 

Our notification stack is going to be [Pushbullet](https://www.pushbullet.com/) for a few reasons:

1. I already have an account
2. It can mirror between phone and desktop, so I'm going to get it quick.
3. It supports link types with one click opening of links.
4. Huginn has a native agent for it installed by default.

We'll pull our data from boring Atom RSS feeds -- reliable, updated fairly quick and standardized. Huginn also supports web scraping for more speed (since RSS feeds can sometimes lag behind) and for sites that do not provide federated access to their data. 
...
If you had this reaction, then read below the requirements, explicit and implicit as I dissect what complications might arise:
...
* **Expected days**: You can run more on the offical docs --  but this is how  Huginn will notify us of broken agents. i.e: we expect data to come in here at least somewhat frequent or something is not right.
...
We now have our own copy of the data using RSS. It could be through scraping the HTML using the scraper agent, too, though. It does not matter -- what does is that we now have _events_. We don't want to send every _Hot Deal_ to our devices since that's just going to result in too many posts. Probably at least three day and we don't want to have to keep checking these. First, [let's do a little bit of research using Google to figure out what we can](https://www.google.ca/search?q=price+error+redflagdeals&oq=price+error+&aqs=chrome.0.69i59j69i60j69i57j0l2j69i61.1261j0j7&sourceid=chrome&ie=UTF-8) expect.
...
(_Trigger Agents_ allow us to filter data and then generate other events based on these. You can think of the raw data as the _base fact_ and then this is a refinement on those facts. Specifically, it generates events we are interested in that we want real time information on. We could still use the raw data for other things since it does not transform it -- for example, a daily digest of all the _Hot Deals_ for the day at the end of a day.)
...
Time to get that info on your phone.
...
.. and you do need to fill in your API key. After that, you should have everything you need. 

# Profit???

That's it! With that, we should now have real-time updates to our devices like we showed above. It all works but you don't have to my word for it -- you can create a _manual agent_ and then trigger some events to watch the entire thing work together. You could also try finding a deal and posting it as well and that would also trigger your new test.

With this, we now have completely automated the process of getting notified of price errors. I've been running a variant of this for a while and scooped up at least a few things such as [Google Home Mini's for $30 to due a coupon error](https://forums.redflagdeals.com/best-buy-update-google-home-mini-now-49-2184059/). The original thread title used to say **HOT** but it was modified since the deal has been expired. It's not perfect, so I often actually combine this with digests at the end of the day to comb through "not error" things but things I still may be interested in.
