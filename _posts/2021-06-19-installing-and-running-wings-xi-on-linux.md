...
3. **The login boot loader keeps repeating the menu over and over and I can't login.** There exists a bug in WINE at the time of writing that prevents you from using the bootloader IO console. You can work around this by editing the boot command from WingsXI with your own command. If you have modified **nothing else** in the XML configuration, you can simply open the file "~/.wine-wingsxi-64/drive_c/WingsXI/Ashita/config/boot/New Configuration 1.xml"` in your favourite text editor and under the `boot_command` tag you can add the following arguments `--user USERNAME --pass PASSWORD` to the command.
